/**
 * 日期格式化（独立工具函数，原型扩展向后兼容委托于此）
 * 月(M)、日(d)、小时(h)、分(m)、秒(s)、季度(q) 可以用 1-2 个占位符，
 * 年(y)可以用 1-4 个占位符，毫秒(S)只能用 1 个占位符(是 1-3 位的数字)
 * 例：formatDate(new Date(), "yyyy-MM-dd hh:mm:ss.S")
 */
...
/**
 * 字符串模板替换（独立工具函数）
 * 支持：数组下标 {0}{1}、对象属性 {name}{age}、对象多层嵌套 {user.name}
 */
 ...
 * 全局正则替换（独立工具函数）
 */
function replaceAll(str, search, target) {
    return str.replace(new RegExp(search, "gm"), target);
}

/**
 * 数组查找索引（独立工具函数）
 */
function getArrayIndex(arr, val, field) {
    if (field) {
        return arr.findIndex((obj) => obj[field] == val);
    }
 ...
 * 数组删除元素（独立工具函数）
 */
function removeArrayItem(arr, val, field) {
    var index = getArrayIndex(arr, val, field);
    if (index > -1) {
        arr.splice(index, 1);
    }
}

/* === 向后兼容：保留原型扩展，内部委托给上面的独立函数 === */
...
String.prototype.format = function(args) {
    ...
    return formatString.apply(null, argsArray);
};
String.prototype.replaceAll = function(search, target) {
    return replaceAll(this, search, target);
};
Array.prototype.getIndex = function(val, field) {
    return getArrayIndex(this, val, field);
};
Array.prototype.remove = function(val, field) {
    removeArrayItem(this, val, field);
};
...
API.Common = {
    ...
    formatWxEmoji(content, type) {
        ...
        for (const _content of contentList) {
            ...
            if (_content.type === 2) {
                const imgUrl = imgRelativePath + "/" + _content.image;
                let res = "<img src='{0}' >".format(imgUrl);
                switch (type) {
                    case 'MD':
                        res = API.Utils.getImagesMarkdown(imgUrl);
                }
                result.push(res);
            }
        }
    ...
    handleCommentBomEvent(event) {
        ...
        switch (eventTarget.moduleName) {
            ...
            case 'Messages':
                ...
                // 实际上需要显示的评论（custom_comments 是 commentlist 的超集，已合并为同一字段）
                comments = message.custom_comments || [];
        ...
        <a href="<%:=API.Common.getUserUrl(item.fuin)%>" target="_blank" class="list-group-item list-group-item-action border rounded">
            <div class="d-flex flex-row bd-highlight">
                <div class="avatar bd-highlight">
                    <img class="lazyload loading rounded-circle" src="../Common/images/loading.gif" data-src="<%:=API.Common.getUserLogoUrl(item.fuin)%>" alt="" style="height: 50px;width: 50px;">
                </div>
...
<div class="list-group">
    ...
    <%for (const item of items) {%>
        <a href="<%:=API.Common.getUserUrl(item.uin)%>" target="_blank" class="list-group-item list-group-item-action border rounded">
            <div class="d-flex flex-row bd-highlight">
                <div class="avatar bd-highlight">
                    <img class="lazyload loading rounded-circle" src="../Common/images/loading.gif" data-src="<%:=API.Common.getUserLogoUrl(item.uin)%>" alt="" style="height: 50px;width: 50px;">
                </div>
                <div class="flex-fill bd-highlight align-self-center ml-3">
        ...
        <div class="d-flex justify-content-start ">
            <div class="comment-avatar">
                <a class="avatar p-0 m-0" target="_blank" href="<%:=API.Common.getUserUrl(comment.poster && comment.poster.id || comment.uin)%>">
                    <img class="lazyload loading w-100 h-100 border rounded-circle" src="../Common/images/loading.gif" data-src="<%:=API.Common.getUserLogoUrl(comment.poster && comment.poster.id || comment.uin)%>" >
                </a>
            </div>
                            ...
                            <div class="d-flex justify-content-start">
                                <div class="comment-avatar">
                                    <a class="avatar p-0 m-0 " target="_blank" href="<%:=API.Common.getUserUrl(reply.poster && reply.poster.id || reply.uin)%>">
                                        <img class="lazyload loading w-100 h-100 border rounded-circle" src="../Common/images/loading.gif" data-src="<%:=API.Common.getUserLogoUrl(reply.poster && reply.poster.id || reply.uin)%>">
                                    </a>
                                </div>
                                <div class="comment-infos ml-2">
        ...
        <div class="d-flex justify-content-start message-infos">
            <div class="message-avatar-img">
                <a class="avatar p-0 m-0" target="_blank" href="<%:=API.Common.getUserUrl(message.uin)%>">
                    <img class="lazyload loading w-100 h-100 border rounded-circle" src="../Common/images/loading.gif" data-src="<%:=API.Common.getUserLogoUrl(message.uin)%>">
                </a>
            </div>
                    ...
                    <%for(let image of message.custom_images){%>
                        ...
                        <%}else{%>
                            <a class="medias-item border message-lightbox" data-idx="<%:=imgIdx%>" data-src="<%:=(image.custom_filepath || image.custom_url)%>" data-sub-html="#<%:= 'QZIMG-'+ message.tid%>">
                                <img class="lazyload loading w-100 h-100" data-id="<%:=image.pic_id%>" src="../Common/images/loading.gif" data-src="<%:=(image.custom_filepath || image.custom_url)%>">
                            </a>
                            <%imgIdx++%>
                        <%}%>
                    <%}%>
                ...
                <div class="d-flex justify-content-start ">
                    <div class="comment-avatar">
                        <a class="avatar p-0 m-0" target="_blank" href="<%:=API.Common.getUserUrl(board.uin)%>">
                            <img class="lazyload loading w-100 h-100 border rounded-circle" src="../Common/images/loading.gif" data-src="<%:=API.Common.getUserLogoUrl(board.uin)%>" >
                        </a>
                    </div>
                    <div class="comment-infos ml-2">
                                ...
                                <div class="d-flex justify-content-start ">
                                    <div class="comment-avatar">
                                        <a class="avatar p-0 m-0" target="_blank" href="<%:=API.Common.getUserUrl(reply.uin)%>">
                                            <img class="lazyload loading w-100 h-100 border rounded-circle" src="../Common/images/loading.gif" data-src="<%:=API.Common.getUserLogoUrl(reply.uin)%>" >
                                        </a>
                                    </div>
                                    <div class="comment-infos ml-2">
                ...
                <div class="d-flex justify-content-start ">
                    <div class="comment-avatar">
                        <a class="avatar p-0 m-0" target="_blank" href="<%:=API.Common.getUserUrl(share.uin)%>">
                            <img class="lazyload loading w-100 h-100 border rounded-circle" src="../Common/images/loading.gif" data-src="<%:=API.Common.getUserLogoUrl(share.uin)%>" >
                        </a>
                    </div>
                    <div class="comment-infos ml-2">
 ...
 <div class="list-group-item list-group-item-action mb-2">
     <div class="align-middle mb-2">
        <a class="avatar" href="<%:=API.Common.getUserUrl(friend.uin)%>" target="_blank" title="访问TA的QQ空间">
            <img class="lazyload loading" src="../Common/images/loading.gif" data-src="<%:=API.Common.getMediaPath(API.Common.getUserLogoUrl(friend.uin), friend.custom_avatar, "Friends_HTML")%>">
        </a>
...
/**
 * 画廊工厂：统一封装 lightGallery 的实例化逻辑，消除各业务模块中的重复代码
 */
API.Gallery = {
     ...
     * 默认评论侧栏 HTML
     */
    COMMENT_MARKUP: '<div id="lg-comment-box" class="lg-comment-box lg-fb-comment-box"><div class="lg-comment-header"><h3 class="lg-comment-title">评论</h3><span class="lg-comment-close lg-icon"></span></div><div class="lg-comment-body"></div></div>',

    /**
     * 在指定 DOM 上实例化多媒体画廊（说说/分享/视频/相册）
     * @param {HTMLElement} galleryDom 画廊容器 DOM
     * @param {string} moduleName 模块名（用于评论侧栏分派：Messages/Shares/Videos/Albums/Favorites）
     * @param {Object} extraOptions 额外的 lightGallery 配置
     * @returns {Object} lightGallery 实例
     */
    createMediaGallery(galleryDom, moduleName, extraOptions) {
        if (galleryDom.galleryIns) {
            return galleryDom.galleryIns;
        }
        galleryDom.moduleName = moduleName;
        API.Common.registerEvents(galleryDom);
        const options = Object.assign({
            plugins: [lgZoom, lgAutoplay, lgComment, lgFullscreen, lgRotate, lgThumbnail, lgVideo],
            mode: 'lg-fade',
            ...
            commentsMarkup: API.Gallery.COMMENT_MARKUP
        }, extraOptions || {});
        const ins = lightGallery(galleryDom, options);
        galleryDom.galleryIns = ins;
        return ins;
    },

    /**
     * 在指定 DOM 上实例化评论图片画廊（无评论侧栏、无视频）
     * @param {HTMLElement} galleryDom 画廊容器 DOM
     * @param {string} selector 点击元素选择器
     * @param {Object} extraOptions 额外的 lightGallery 配置
     * @returns {Object} lightGallery 实例
     */
    createCommentGallery(galleryDom, selector, extraOptions) {
        if (galleryDom.galleryIns) {
            return galleryDom.galleryIns;
        }
        const options = Object.assign({
            ...
            mode: 'lg-fade',
            selector: selector || '.comment-img-lightbox',
            download: false,
            thumbnail: false,
            loop: false
        }, extraOptions || {});
    ...
    /**
     * 绑定多媒体画廊点击事件（标准 .message-lightbox 模式）
     * @param {string} containerSelector 容器选择器（默认 .lightgallery）
     * @param {string} moduleName 模块名
     * @param {Object} extraOptions 额外配置
     */
    bindMediaGallery(containerSelector, moduleName, extraOptions) {
        $(containerSelector + ' .message-lightbox').on('click', function() {
            const $galleryDom = $(this).parent().get(0);
            const imgIdx = $(this).attr('data-idx');
            const ins = API.Gallery.createMediaGallery($galleryDom, moduleName, extraOptions);
    ...
    /**
     * 绑定评论图片画廊点击事件
     * @param {string} itemSelector 点击元素选择器（默认 .comment-img-lightbox）
     */
    bindCommentGallery(itemSelector) {
        itemSelector = itemSelector || '.comment-img-lightbox';
        $(itemSelector).on('click', function() {
            const $galleryDom = $(this).parent().parent().get(0);
            const imgIdx = $(this).attr('data-idx');
            const ins = API.Gallery.createCommentGallery($galleryDom, itemSelector);
            ins.openGallery(imgIdx * 1);
        });
    }
};
...
/**
 * 日志/日记列表+详情页工厂，消除 blogs.js / diaries.js / bloginfo.js / diaryinfo.js 的重复
 */
API.BlogLike = {

    /**
     * 默认表格列定义（与原 blogs.js 保持一致）
     */
    DEFAULT_TABLE_COLUMNS: [{
        ...
        width: "50",
        sortable: true,
        formatter: (value, row) => API.Blogs.getBlogLabel(row).join(',')
    }, {
        field: 'pubtime',
        title: '发布时间',
        align: 'center',
        width: "150",
        sortable: true,
        formatter: (value) => API.Utils.formatDate(value)
    }, {
        field: 'replynum',
        title: '评论数',
        align: 'center',
    ...
    /**
     * 渲染列表（HTML / Table 两种视图）
     * @param {string} dataVar 全局数据变量名（blogs / diaries）
     * @param {string} configKey QZone_Config 中的键（Blogs / Diaries）
     * @param {string} tableId 表格 DOM ID（如 blogs-table / diaries-table）
     */
    initList(dataVar, configKey, tableId) {
        const config = QZone_Config[configKey] || {};
        const data = window[dataVar] || [];
        if (config.showType === '0') {
            API.BlogLike.showTableList(data, tableId || (dataVar + '-table'));
        } else {
            API.BlogLike.showList(data);
        }
    },
     ...
     */
    showList(data) {
        const categoryMaps = API.Utils.groupedByField(data, 'category');
        const list_html = template(TPL.BLOGS_TYPE_LIST, { blogs: data, categoryMaps });
     ...
     * 渲染表格视图
     */
    showTableList(data, tableId) {
        API.Utils.initTable(tableId, API.BlogLike.DEFAULT_TABLE_COLUMNS, data);
    },

    /**
     * 渲染日志/日记详情页（共用逻辑）
     * @param {string} dataVar 全局数据变量名
     */
    initInfo(dataVar) {
        const blogId = API.Utils.getUrlParam('blogId');
        if (!blogId) {
            $('#blog_title').text('未指定 ID');
            return;
        }
        const data = window[dataVar] || [];
        const idx = data.getIndex(blogId * 1, 'blogid');
        if (idx < 0) {
            $('#blog_title').text('未找到 ID=' + blogId + ' 的记录');
            return;
        }
        const blog = data[idx];
        document.title = 'QQ空间备份-' + (blog.custom_title || blog.title || '');
        $('#blog_title').text(blog.custom_title || blog.title || '');
        $('#blog_time').text(API.Utils.formatDate(blog.pubtime));
        $('#blog_content').html(API.Utils.base64ToUtf8(blog.custom_html || ''));

        // 评论模板（页面内 #comments_tpl）
        const commentsTplEl = document.getElementById('comments_tpl');
        if (commentsTplEl) {
            const comments_html = template(commentsTplEl.innerHTML, { blog });
        ...
        $('#blog_content img').on('click', function() {
            ...
            const imgIdx = $(this).attr('data-idx') || $(this).index();
        ...
        API.Common.registerShowVisitorsWin(data);
        API.Common.registerShowLikeWin(data);
    }
};

/**
 * 主题切换（P2-16）：在页面右下角注入按钮，记忆用户偏好到 localStorage
 */
API.Theme = {
    STORAGE_KEY: 'qz-theme',
    init() {
        // 应用已保存的主题（避免闪烁：在 <head> 中也可手动提前调用）
        const saved = localStorage.getItem(API.Theme.STORAGE_KEY);
        ...
        const $btn = $('<button class="theme-toggle-btn" aria-label="切换主题"><i class="fa fa-moon-o"></i></button>');
        $('body').append($btn);
        API.Theme._refreshIcon($btn);
        $btn.on('click', function() {
            const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
            if (isDark) {
                document.documentElement.removeAttribute('data-theme');
                localStorage.setItem(API.Theme.STORAGE_KEY, 'light');
            } else {
                document.documentElement.setAttribute('data-theme', 'dark');
                localStorage.setItem(API.Theme.STORAGE_KEY, 'dark');
            }
            API.Theme._refreshIcon($btn);
        });
    },
    _refreshIcon($btn) {
        const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
        $btn.find('i').toggleClass('fa-moon-o', !isDark).toggleClass('fa-sun-o', isDark);
    }
};
...
/**
 * 全站搜索（P2-15）：在所有页面注入搜索按钮，跨已加载模块数据做全文匹配
 */
API.Search = {
    // 各模块数据源与字段映射（仅当对应全局变量存在时才搜索）
    SOURCES: [
        { name: '说说', varPath: 'messages', fields: ['name', 'conlist'], timeField: 'created_time', linkPrefix: 'Messages/index.html#' },
        { name: '留言', varPath: 'boardInfo.items', fields: ['nickname', 'ubbContent', 'htmlContent'], timeField: 'pubtime', linkPrefix: 'Boards/index.html#' },
        { name: '日志', varPath: 'blogs', fields: ['title', 'custom_title'], timeField: 'pubtime', linkPrefix: 'Blogs/index.html?blogId=' },
        { name: '私密日记', varPath: 'diaries', fields: ['title', 'custom_title'], timeField: 'pubtime', linkPrefix: 'Diaries/index.html?blogId=' },
        { name: '分享', varPath: 'shares', fields: ['nickname', 'desc'], timeField: 'shareTime', linkPrefix: 'Shares/index.html#' }
    ],
    ...
    _extractText(item, fields) {
        ...
        fields.forEach((f) => {
            ...
            if (typeof v === 'string') {
                ...
                v.forEach((c) => {
                    if (c && typeof c.con === 'string') parts.push(c.con);
                });
            }
        });
        // 去除 HTML 标签与表情占位符
        return parts.join(' ').replace(/<[^>]+>/g, '').replace(/\[em\]e\d+\[\/em\]/g, '').replace(/\s+/g, ' ');
    },
    ...
    search(keyword) {
        ...
        if (!kw) return [];
        const results = [];
        API.Search.SOURCES.forEach((src) => {
            const data = API.Search._getValueByPath(window, src.varPath);
            if (!Array.isArray(data)) return;
            data.forEach((item) => {
                const text = API.Search._extractText(item, src.fields).toLowerCase();
                if (text.indexOf(kw) === -1) return;
                const idx = text.indexOf(kw);
                const start = Math.max(0, idx - 20);
                const end = Math.min(text.length, idx + kw.length + 40);
                const snippet = (start > 0 ? '…' : '') + text.substring(start, end) + (end < text.length ? '…' : '');
                const time = item[src.timeField];
                results.push({
                    source: src.name,
                    snippet,
                    time: time ? API.Utils.formatDate(time) : '',
                    link: src.linkPrefix + (item.tid || item.id || item.blogid || item.blogId || '')
                });
            });
        });
        return results;
    ...
    init() {
        if ($('.search-toggle-btn').length > 0) return;
        $('body').append('<button class="search-toggle-btn" aria-label="搜索"><i class="fa fa-search"></i></button>');
        $('body').append(
            '<div class="modal fade search-modal" tabindex="-1" role="dialog">' +
            '  <div class="modal-dialog modal-lg" role="document">' +
            '    <div class="modal-content">' +
            '      <div class="modal-header">' +
            '        <h5 class="modal-title"><i class="fa fa-search mr-2"></i>全站搜索</h5>' +
            '        <button type="button" class="close" data-dismiss="modal" aria-label="关闭"><span>&times;</span></button>' +
            '      </div>' +
            '      <div class="modal-body">' +
            '        <div class="input-group mb-3">' +
            '          <input type="text" class="form-control search-input" placeholder="输入关键词，跨说说/留言/日志/分享检索…">' +
            '          <div class="input-group-append"><button class="btn btn-primary search-submit">搜索</button></div>' +
            '        </div>' +
        ...
        const doSearch = () => {
            ...
            const results = API.Search.search(kw);
            const $box = $('.search-results');
            if (results.length === 0) {
                $box.html('<div class="text-muted text-center py-4">未找到匹配结果</div>');
                return;
            }
            const html = results.slice(0, 50).map((r) => (
                '<a href="' + r.link + '" class="list-group-item list-group-item-action">' +
                '<div class="d-flex justify-content-between"><span class="badge badge-secondary">' + r.source + '</span><small class="text-muted">' + r.time + '</small></div>' +
                '<div class="search-snippet mt-1">' + r.snippet + '</div>' +
                '</a>'
            )).join('');
            $box.html('<div class="list-group">' + html + '</div>' + (results.length > 50 ? '<div class="text-muted text-center mt-2">仅显示前 50 条，共 ' + results.length + ' 条</div>' : ''));
        };
        $('.search-submit').on('click', doSearch);
...
/**
 * 隐私脱敏（P3-21）
 * 默认启用；URL 含 ?debug=1 时禁用，展示原始数据
 */
API.Privacy = {
    isEnabled() {
        return API.Utils.getUrlParam('debug') !== '1';
    },
    /**
     * QQ 号脱敏：保留前 3 位与最后 1 位，中间用 **** 替换
     */
    maskUin(uin) {
        if (!API.Privacy.isEnabled() || !uin) return uin;
        const s = String(uin);
        if (s.length <= 5) return s;
        return s.substring(0, 3) + '****' + s.substring(s.length - 1);
    },
    /**
     * 昵称脱敏：保留首尾各 1 字符，中间用 * 替换（小于 3 字符不动）
     * 空间主人昵称不脱敏
     */
    maskNick(uin, nick) {
        if (!API.Privacy.isEnabled() || !nick) return nick;
        if (window.userInfo && String(uin) === String(userInfo.uin)) return nick;
        const s = String(nick);
        if (s.length < 3) return s;
        return s.charAt(0) + '*'.repeat(Math.min(s.length - 2, 4)) + s.charAt(s.length - 1);
    }
};

// 接入 getUserLink：他人昵称脱敏（在原方法外层包装）
(function() {
    const _originGetUserLink = API.Common.getUserLink;
    API.Common.getUserLink = function(uin, nickName, type) {
        return _originGetUserLink.call(this, uin, API.Privacy.maskNick(uin, nickName), type);
    };
})();